        ...
        return this.each(function () {
            ...
            this.createHtml = function () {
                ...
                if (para.dragDrop) {
                    ...
                    html += '<input type="hidden" name="_csrf" value="' + $('meta[name="csrf-token"]').attr('content') + '"><input type="hidden" name="_is_ajax" value="1" />';
                    ...
                    // 创建不带有拖动的html
                    html += '<form id="uploadForm" name="uploadForm" action="' + para.url + '" method="post" enctype="multipart/form-data">';
                    html += '<input type="hidden" name="_csrf" value="' + $('meta[name="csrf-token"]').attr('content') + '"><input type="hidden" name="_is_ajax" value="1" />';
                    html += '	<div class="upload_box">';
                    html += '		<div class="upload_main single_main">';
                    html += '			<div class="status_bar hide">';
                    html += '				<div id="status_info" class="info hide">选中0张文件，共0B。</div>';
                    html += '				<div class="btns">';
                    html += '					<input id="fileImage" type="file" size="30" name="option_thumb" ' + multiple + '>';
                    ...
                    html += '						<div class="uploadImg andArea" style="width:' + imgWidth + 'px">';
            ...
            this.funFilterEligibleFile = function (files) {
                ...
                for (var i = 0, file; file = files[i]; i++) {
                    var obj = file.type;
                    console.log($.inArray(obj, type));
                    if (file.size >= 51200000) {
                        alert('您这个"' + file.name + '"文件大小过大');
                    } else if ($.inArray(obj, type) < 0) {
