 ...
 */
abstract class DbOrmModel extends Model {
    ...
    public static function findOne($xCondition) {
        if (is_numeric($xCondition)) {
            ...
            throw new InvalidParamException('错误的查询条件');
        }
        $aData = (new Query())->from(static::tableName())->where($xCondition)->one();
        if (!$aData) {
            return false;
        }
    ...
    protected static function _beforeFindAll(Query $oQuery, $aCondition) {
        //加上软删除
        if (isset(static::$_aSoftDelete[0]) && isset(static::$_aSoftDelete[1])) {
            $oQuery->andWhere(['<>',static::$_aSoftDelete[0] ,static::$_aSoftDelete[1]]);
        }
        return $oQuery;
    }
    ...
    public static function tableName() {
        throw new InvalidCallException('数据库ORM模型必须定义 public static function tableName 方法返回主表名');
    }
    ...
    public function delete() {
        //软删除
        if (isset(static::$_aSoftDelete[0]) && isset(static::$_aSoftDelete[1])) {
            $this->set(static::$_aSoftDelete[0], static::$_aSoftDelete[1]);
            return $this->save();
        }
        //物理删除
    ...
    /**
     * 批量删除(根据条件)
     * @param type $xCondition 
     * @return int 成功删除的条数
     */
    public static function deleteAll($xCondition) {
        if (is_numeric($xCondition)) {
            $xCondition = ['id' => (int) $xCondition];
        } elseif (!is_array($xCondition)) {
            throw new InvalidParamException('错误的查询条件');
        }
        $aModelList = static::findAll($xCondition);
        $successCount = 0;
        foreach ($aModelList as $aModel) {
            $mModel = static::toModel($aModel);
            if (!$mModel) {
                continue;
            }
            if ($mModel->delete()) {
     ...
     * 批量更新(根据条件)
     * @param type $xCondition 
     * @param type $aUpdateData 要更新的数据 ['status'=>1,'xxx'=>'xx'] 
     * @return int 成功更新的条数
     */
    public static function saveAll($xCondition, $aUpdateData) {
        if (is_numeric($xCondition)) {
            $xCondition = ['id' => (int) $xCondition];
        } elseif (!is_array($xCondition)) {
            throw new InvalidParamException('错误的查询条件');
    ...
    public static function getList($aCondition = [], $aControl = [], $aFeilds = []) {
        $aWhere = static::parseWhereCondition($aCondition);
        $page = 0;
        $pageSize = 0;
        $aSortList = [];
        if (isset($aControl['order_by'])) {
            $aSortList = $aControl['order_by'];
        }
        if (isset($aControl['page']) && isset($aControl['page_size'])) {
            $page = $aControl['page'];
            $pageSize = $aControl['page_size'];
        }
        return static::findAll($aWhere, $aFeilds, $page, $pageSize, $aSortList);
    }
    ...
    public static function getCount($aCondition = []) {
        $aWhere = static::parseWhereCondition($aCondition);
         //加上软删除
         if (isset(static::$_aSoftDelete[0]) && isset(static::$_aSoftDelete[1])) {
            $aWhere[] = ['<>', static::$_aSoftDelete[0], static::$_aSoftDelete[1]];
        }
