...
class Alipay extends \yii\base\Object{
	/**
	 * partnerID(应用ID)
	 */
	public $partner_id = '';
	...
	/**
	 * 生成签名结果
	 * @param $aParaSort 已排序要签名的数组
	 * return 签名结果字符串
	 */
	private function buildRequestMysign($aParaSort) {
		//把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
		$prestr = $this->_createLinkstring($aParaSort);

		$mysign = "";
		switch (strtoupper(trim($this->alipay_config['sign_type']))) {
			case "MD5" :
				$mysign = $this->_md5Sign($prestr, $this->alipay_config['key']);
				break;
			default :
				$mysign = "";
		}
	...
	/**
	 * 生成要请求给支付宝的参数数组
	 * @param $aParaTemp 请求前的参数数组
	 * @return 要请求的参数数组
	 */
	private function buildRequestPara($aParaTemp) {
		//除去待签名参数数组中的空值和签名参数
		$aParaFilter = $this->_paraFilter($aParaTemp);

		//对待签名参数数组排序
		$aParaSort = $this->_argSort($aParaFilter);

		//生成签名结果
		$mysign = $this->buildRequestMysign($aParaSort);

		//签名结果与签名方式加入请求提交参数组中
		$aParaSort['sign'] = $mysign;
		$aParaSort['sign_type'] = strtoupper(trim($this->alipay_config['sign_type']));
	 ...
	 * @return 要请求的参数数组字符串
	 */
	public function buildRequestParaToString($aParaTemp) {
		//待请求参数数组
		$aPara = $this->buildRequestPara($aParaTemp);

		//把参数组中所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串，并对字符串做urlencode编码
		$requestData = createLinkstringUrlencode($aPara);
	...
	/**
	 * 建立请求，以表单HTML形式构造（默认）
	 * @param $aParaTemp 请求参数数组
	 * @param $method 提交方式。两个值可选：post、get
	 * @param $button_name 确认按钮显示文字
	 * @return 提交表单HTML文本
	 */
	public function buildRequestForm($aParaTemp, $method, $button_name) {
		//待请求参数数组
		$para = $this->buildRequestPara($aParaTemp);
	 ...
	 * @return 支付宝处理结果
	 */
	public function buildRequestHttp($aParaTemp) {
		$sResult = '';

		//待请求参数数组字符串
		$request_data = $this->buildRequestPara($aParaTemp);
	 ...
	 * @return 支付宝返回处理结果
	 */
	public function buildRequestHttpInFile($aParaTemp, $file_para_name, $file_name) {
	...
	public function verifyNotify(){
		...
		if(empty($aPost)){//判断POST来的数组是否为空
			return false;
		}else{
			//生成签名结果
			$sign = Yii::$app->request->post('sign');
			$isSign = $this->getSignVeryfy($aPost, $sign);
	 ...
	 */
	private function _paraFilter($aPara) {
		$aParaFilter = array();
		while (list ($key, $val) = each ($aPara)) {
			if($key == "sign" || $key == "sign_type" || $val == ""){
				continue;
			}else{
				$aParaFilter[$key] = $aPara[$key];
			}
		}
		return $aParaFilter;
	}
