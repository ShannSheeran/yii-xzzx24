...
class Query extends \yii\db\Query{
	 ...
	 */
	public $isCacheData = false;		//是否要从缓存中查，通过表配置得到
	private $_select = null;		//保存最开始要查询的字段信息
	public $isSelectFromCache = false;	//本次query是否查缓存，如果是false则不查缓存。是true则有可能查缓存，看表配置
	...
	private function _isSelectFromRedis(){
		...
		$isSelectRedis = true;
		if(!$oRedis->isOpen || !$oRedis->servers[$oRedis->serverName]['is_active'] || $this->groupBy || $this->orderBy || $specialFlag || !$this->isCacheData || $this->join || $this->union || !$this->isSelectFromCache){
			$isSelectRedis = false;
		}
