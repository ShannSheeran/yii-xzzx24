...
<div id="uploader">
        ...
        <script>
            ...
            function uploaded(ret, file) {
                console.log(file.id);
                $('#' + file.id).attr('data-md5', file.md5).attr('data-src', ret);
                top.$('[name="<?php echo $field ?>"]').map(function () {
                    top.$(this).attr('data-srcs', ret).attr('data-md5', file.md5).val(ret).trigger('change');
                });
                //top.$.msg.tips('文件上传成功！');
            ...
            $(function () {
                ...
                if (!WebUploader.Uploader.support('flash') && WebUploader.browser.ie) {
                    ...
                    if (flashVersion) {
                        (function (container) {
                            ...
                            var swf = '<?php echo Yii::getAlias('@r.swf.uploader.uploader'); ?>';
                ...
                uploader = WebUploader.create({
                    ...
                    <?php if(in_array($ext, $aFildList)){?>
                     accept: {
                        title: '选择文件',
                        extensions: 'zip,xls',
                        mimeTypes: 'application/x-zip,application/zip,application/x-zip-compressed,application/s-compressed,multipart/x-zip'
                    },
                    <?php }else{?>
                    accept: {
                        title: '选择文件',
                        extensions: 'gif,jpeg,jpg,bmp,png', //'gif,jpg,jpeg,bmp,png',
                        mimeTypes: 'image/gif,image/jpeg,image/pjpeg,image/jpeg,image/pjpeg,image/bmp,image/x-bmp,image/x-bitmap,image/x-xbitmap,image/x-win-bitmap,image/x-windows-bmp,image/ms-bmp,image/x-ms-bmp,application/bmp,application/x-bmp,application/x-win-bitmap,image/png,image/x-png'
                    },
                    <?php }?>
                    formData: {
                    ...
                    dnd: '#dndArea',
                    paste: document.body,
                    swf: '<?php echo Yii::getAlias('@r.swf.uploader.uploader'); ?>',
                    chunked: false,
                    chunkSize: 512 * 1024,
                    server: '<?php echo Url::to(['plugs/upload-file','uptype' => $uptype, 'type' => $type, 'save_path' => $savePath, 'field' => $field,'ext'=>$ext])?>',
                    ...
                    if (file.getStatus() === 'invalid') {
                        ...
                        uploader.makeThumb(file, function (error, src) {
                            ...
                            if (isSupportBase64) {
                                ...
                                $.ajax('http://xzzx24.dev/index.php/plugs/file/preview.html', {
                                    method: 'POST',
                                    data: src,
                                    dataType: 'json'
